# This Makefile is for the Text::Naming::Convention extension to perl.
#
...
#     ABSTRACT => q[Naming or Renaming( for identifiers, mostly )]
#     AUTHOR => q[sunnavy <sunnavy@bestpractical.com>]
#     DIR => []
#     DISTNAME => q[Text-Naming-Convention]
#     NAME => q[Text::Naming::Convention]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], ExtUtils::MakeMaker=>q[6.44] }
#     VERSION => q[0.0.5]
#     dist => { PREOP=>q[$(PERL) -I. "-MModule::Install::Admin" -e "dist_preop(q($(DISTVNAME)))"] }
#     test => { TESTS=>q[t/00.load.t t/01.api.t t/kwalitee.t t/pod-coverage.t t/pod.t] }
...
NAME_SYM = Text_Naming_Convention
VERSION = 0.0.5
...
MAKE = make
FULLEXT = Text/Naming/Convention
BASEEXT = Convention
PARENT_NAME = Text::Naming
DLBASE = $(BASEEXT)
VERSION_FROM = 
OBJECT = 
LDFROM = $(OBJECT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Text/Naming/Convention.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Text/Naming
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Text/Naming
...
TO_INST_PM = lib/Text/Naming/Convention.pm

PM_TO_BLIB = lib/Text/Naming/Convention.pm \
...
DISTNAME = Text-Naming-Convention
DISTVNAME = Text-Naming-Convention-0.0.5
...
manifypods : pure_all  \
	lib/Text/Naming/Convention.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Text/Naming/Convention.pm $(INST_MAN3DIR)/Text::Naming::Convention.$(MAN3EXT) 
...
TEST_FILE = test.pl
TEST_FILES = t/00.load.t t/01.api.t t/kwalitee.t t/pod-coverage.t t/pod.t
TESTDB_SW = -d
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Naming or Renaming( for identifiers, mostly )</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>sunnavy &lt;sunnavy@bestpractical.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  lib/Text/Naming/Convention.pm blib/lib/Text/Naming/Convention.pm 
